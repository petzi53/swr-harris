...
I learned to use {**purrr**}-like formulas for creating a function on the spot. Important for my understanding was the [help file for `tidyselect::where()`](https://tidyselect.r-lib.org/reference/where.html#the-formula-shorthand) and the StackOverflow answer [Rounding selected columns of a data frame](https://stackoverflow.com/a/27613376/7322615).
...
To experiment with realistic data I will use the `bdims` dataset of {**openintro**}. It is a body measurement of 507 physically active individuals (247 men and 260 women). To get the following code to run you need to install the {**openintro**} via CRAN.
...
- elbow diameter, measured as sum of two elbows
...
| n > 300        |     under -7 or over 7    | Visual inspection recommended!  |
...
## construct graph using my private function hist_dnorm()
p <- hist_dnorm(bdims_male, bdims_male$bia_di)
...
Checking the skewness of the shoulder breadth distribution of 247 men with `semTools::skew()` and `statpsych::test.skew()`
...
## construct graph using my private function hist_dnorm()
p <- hist_dnorm(bdims, bdims$bii_di)
...
## construct graph using my private function hist_dnorm()
p <- hist_dnorm(bdims, bdims$wai_gi)

p +
    ggplot2::labs(x = "Respondent's waist girth in centimeters, measured at the narrowest part of torso below the rib cage as average of contracted and relaxed position")
...
## construct graph using my private function hist_dnorm()
p <- hist_dnorm(bdims, bdims$bit_di)
...
#| fig-cap: "Bitrochanteric diameter (shoulder breadth) distribution of 507 individuals"
#| cache: true
#| results: hold

## construct graph using my private function hist_dnorm()
p <- hist_dnorm(bdims, bdims$bia_di)

p +
    ggplot2::labs(x = "bitrochanteric diameter distribution of 507 individuals")

glue::glue("###################################################################")
glue::glue("Kurtosis of bitrochanteric diameter using `semTools::kurtosis()`")
semTools::kurtosis(bdims$bia_di)
glue::glue(" ")
glue::glue("###################################################################")
glue::glue("Kurtosis of bitrochanteric diameter using `statpsych::test.kurtosis()`")
...
Hmisc package for miscellaneous functions for data analysis and related tasks
...
{**modeest**} Package for Mode Estimation
:::
