...
{**autoplotly**}: [Automatic Generation of Interactive Visualizations for Statistical Results](https://github.com/terrytangyuan/autoplotly) [@autoplotly]
...
{**dfidx**}: [Indexed Data Frames](https://cran.r-project.org/package=dfidx) [@dfidx]


(*There is no hexagon logo for {**dfidx**} available*)
...
{**ggfortify**}: [Data Visualization Tools for Statistical Analysis Results](https://github.com/sinhrks/ggfortify) [@ggfortify]


(*There is no hexagon logo for {**ggfortify**} available*)
...
The packages has for this book  to be loaded explicitly. Calling functions with the `ggfortify::` syntax will not work, because {**ggfortify**} needs (and loads) {**ggplot**}.
***
::::
:::::
...
{**MASS**}: [Package ‘MASS’](http://www.stats.ox.ac.uk/pub/MASS4/) [@MASS]


(*There is no hexagon logo for {**MASS**} available*)
...
{**mlogit**}: [Multinomial Logit Models](https://cran.r-project.org/package=mlogit) [@mlogit]


(*There is no hexagon logo for {**mlogit**} available*)
...
{**nhstplot**}: [Plot Null Hypothesis Significance Tests](https://cran.r-project.org/package=nhstplot) [@nhstplot]
...
{**nnet**}: [Feed-Forward Neural Networks and Multinomial Log-Linear Models](http://www.stats.ox.ac.uk/pub/MASS4/) [@nnet]


(*There is no hexagon logo for {**nnet**} available*)
...
:::::{.my-package}
:::{.my-package-header}
odds.n.ends: Odds Ratios, Contingency Table, and Model Significance from a Generalized Linear Model 
:::
::::{.my-package-container}
...
{**odds.n.ends**}: [Odds Ratios, Contingency Table, and Model Significance from a Generalized Linear Model](https://cran.r-project.org/package=odds.n.ends)  [@odds.n.ends]
...
{**ordinal**}: [Regression Models for Ordinal Data](https://github.com/runehaubo/ordinal) [@ordinal]

:
(*There is no hexagon logo for {**ordinal**} available*)
...
{**performance**}: [Assessment of Regression Models Performance](https://easystats.github.io/performance/) [@performance]
...
{**pscl**}: [Political Science Computational Laboratory](https://github.com/atahk/pscl) [@pscl]

(There is no hexagon logo for {**pscl**} available)
...
{**pubh**}: [A Toolbox for Public Health and Epidemiology](https://cran.r-project.org/package=pubh) [@pubh]

(There is no hexagon logo for {**pubh**} available)
...
{**rms**}: [Regression Modeling Strategies](https://hbiostat.org/r/rms/) [@rms]

(There is no hexagon logo for {**rms**} available)
