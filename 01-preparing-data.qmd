...
- Introducing {**gtsummary**}, a packages for presentation-ready data summary and analytic result tables (@sec-chap01-intro-gtsummary)
...
chap01_nhanes_2013_grouped <- chap01_nhanes_2013_clean |> 
    ...
    dplyr::select(grass, gender, age_cat)
...
:::::{.my-r-code}
:::{.my-r-code-header}
:::::: {#cnj-chap01-grass-age-gender}
: Percentage of NHANES participants 2013 survey by age group and gender answering yes and no to marijuana use
::::::
:::
::::{.my-r-code-container}

```{r}
#| label: fig-grass-age-gender
#| fig-cap: "Proportion of marijuana use of the participant by age group and gender in the NHANES 2013 survey"
...
@fig-grass-age-gender is a little difficult to interpret. Both bars colors shows the proportion of "Yes"-respondents in the appropriate age group. For instance: About 62% of males and about 51% of women of the age group 18-29 said "Yes". 
...
To distinguish the gender colors in @fig-grass-age-gender from the Yes/No colors in @fig-grass-age-group1 and @fig-grass-age-group2 I chose at first some kind of blue for men and red for females. But I learned from [An alternative to pink & blue: Colors for gender data](https://blog.datawrapper.de/gendercolor/) that one should avoid this stereotypical colors. 
...
::: {#int-chap01-gss-nhanes .callout-tip}
...
With the exception of the age group 40-49 the proportion of males ever used in their respective age group is higher than the percentage of women. (See @fig-grass-age-gender). But this is only a snapshot and difficult to interpret. Another possible thesis could be that there is a tiny upward trend from old top young in marijuana use. One possible explanation for the untypical higher use in the oldest age group could be that people between 50-59 of the 2013 survey were born between 1954-1963 and were young adults during the time of the [Flower Power](https://en.wikipedia.org/wiki/Flower_power) generation part of the [counterculture of the 1960s](Counterculture of the 1960s)-1970s.
...
:::
::::{.my-note-container}
{**DT**} is my first choice for interactive data exploring and very useful for Explorative Data Analysis (`r glossary("EDA")`). I have already some experiences with this package but haven't use it lastly. 
...
### Introduction to the gtsummary package {#sec-chap01-intro-gtsummary}
...
chap01_nhanes_2013_clean <- 
    base::readRDS("data-clean/chap01_nhanes_2013_clean.rds")
...
base::saveRDS(chap01_gtsummary_basic, "data-clean/chap01_gtsummary_clean.rds")

base::summary(chap01_gtsummary_basic)
...
chap01_gtsummary_basic <- 
    base::readRDS("data-clean/chap01_gtsummary_clean.rds")
...
## run only once (manually)
chap01_gss_grass_legal <- gss_all |> 
    dplyr::select(year, age, grass) 

base::saveRDS(chap01_gss_grass_legal, "data-raw/chap01_gss_grass_legal.rds")
```
...
## run only once (manually)
chap01_gss_grass_legal <- 
    base::readRDS("data-raw/chap01_gss_grass_legal.rds")

chap01_gss_grass_legal_clean <- chap01_gss_grass_legal |> 
    tidyr::drop_na() |> 
    dplyr::mutate(
...
base::saveRDS(chap01_gss_grass_legal_clean, file = 
                "data-clean/chap01_gss_grass_legal_clean.rds")
```
...
grass_legal <- 
    base::readRDS("data-clean/chap01_gss_grass_legal_clean.rds")
...
grass_legal2 |> 
    ...
    ggplot2::scale_x_continuous(
        # breaks = c(1973, 1978, 1984, 1988, 1991,
        #            1996, 2002, 2008, 2014, 2022)
        # breaks = c(1973, 1976, 1980, 1984, 1987, 1989,
