...
chap01_gss_2016 <- readr::read_csv(
    file = "data-harris/legal_weed_age_GSS2016_ch1.csv",
    show_col_types = FALSE)


chap01_gss_2016_clean <- chap01_gss_2016 |> 
...
saveRDS(chap01_gss_2016_clean, "data-clean/chap01_gss_2016_clean.rds")

## (C) check result #########

## summarize
print("************************   SUMMARY   ****************************")
base::summary(chap01_gss_2016_clean)

## glance at the data
print("*******************   GLANCE AT SOME DATA  **********************")
chap01_gss_2016_clean |>
...
chap01_gss_2016_clean <-  readRDS("data-clean/chap01_gss_2016_clean.rds")
...
chap01_nhanes_2013_clean <- nhanes_2013_ch1 |> 
    dplyr::rename(nr = ...1) |> 
    dplyr::rename(
        gender = RIAGENDR,
        age = RIDAGEYR,
...
## save cleaned data ###############
saveRDS(chap01_nhanes_2013_clean, "data-clean/chap01_nhanes_2013_clean.rds")

base::summary(chap01_nhanes_2013_clean)
skimr::skim(chap01_nhanes_2013_clean)
```
...
chap01_nhanes_2013_clean <- readRDS("data-clean/chap01_nhanes_2013_clean.rds")

tibble::enframe(table(chap01_nhanes_2013_clean$grass)) |> 
...
ggplot2::ggplot(chap01_nhanes_2013_clean,
    ggplot2::aes(grass, 
         y = 100 * ggplot2::after_stat(count) /
             base::sum(ggplot2::after_stat(count)),
             fill = grass
...
chap01_nhanes_2013_grouped <- chap01_nhanes_2013_clean |> 
...
tibble::enframe(table(chap01_nhanes_2013_grouped$age_cat)) |>  
    dplyr::rename(`Ever used grass or hash` = 1, Freq = 2) |> 
    dplyr::mutate(Perc = round(100 * Freq / sum(Freq), 2))

saveRDS(chap01_nhanes_2013_grouped, "data-clean/chap01_nhanes_2013_grouped.rds")
```
...
```{r}
#| label: fig-grass-bar-chart-age-group
#| fig-cap: "Proportion of marijuana use of the participant by age group in the NHANES 2013 survey"

chap01_nhanes_2013_grouped <- readRDS("data-clean/chap01_nhanes_2013_grouped.rds")

## age groups bar chart ########
chap01_nhanes_2013_grouped |> 
  dplyr::group_by(grass, age_cat) |>
  dplyr::count() |> 
  dplyr::group_by(age_cat) |> 
  dplyr::mutate(perc_grass = 100 * n / base::sum(n)) |> 
...
```{r}
#| label: fig-grass-bar-chart-age-sex
...
### Review several table packages

Creating tables is an important result of preparing data --- the main subject of this chapter. In my research I came up with six packages that seems to me the most important ones. What follows is a short introduction into these packages.

:::::{.my-experiment}
:::{.my-experiment-header}
:::::: {#def-chap01-review-table-packages}
: Review table packages: {**DT**}, {**flextable**}, {**gt**}, {**htmlTable**}, {**kableExtra**}, {**xtable**}
::::::
:::
::::{.my-experiment-container}
...
:::
::::{.my-note-container}
{**DT**} is my first choice for interactive data exploring and very useful for Explorative Data Analysis (`r glossary("EDA")`). I have already some experiences with this package but haven't use it lastly. After learning {**gt**} together with {**gtsummary**} I plan to use it more.
::::
:::::
...
:::
::::{.my-note-container}
{**flextable**} seems pretty well suited to create sophisticated tables for publications. Maybe I will look into this package in the near future, but currently my priority is {**gt**} together with {**gtsummary**}.
::::
:::::

###### gt / gtsummary

 [{**gt**}](https://gt.rstudio.com/) & [{**gtsummary**}](https://www.danieldsjoberg.com/gtsummary/index.html)
...
Concerning the supported output format I got inconsistent information. Some information says "Currently {**gt**} mainly supports HTML output." [@xie2020, p.191] But the {**gt**} website states: "Currently, gt supports the HTML, LaTeX, and RTF output formats" (https://gt.rstudio.com/). And {**gtsummary**}, a package that "was written to be a companion to the {**gt**} package from RStudio. But not all output types are supported by the {**gt**} package. Therefore, we have made it possible to print {**gtsummary**} tables with various engines. ([gtsummary + Quarto/R Markdown](https://www.danieldsjoberg.com/gtsummary/articles/rmarkdown.html))
...
{**gt**} is developed by the RStudio/Posit team, that stands not
    only for high quality but also for {**tidyverse**} compatibility. It was a little surprise for me that the download statistics does not reflect the normally very high figures of RStudio/Posit products.
...
:::
::::{.my-note-container}
{**gt**} (together with {**gtsummary**} seems extremely well suited for creating complex tables in many formats. I will explore these two packages in this chapter to learn how to produce a complex table on marijuana use by age group and sex. Learning {**gt**} (together with {**gtsummary**} are one of my priorities for the next time. 
::::
:::::
...
I found {**htmlTable**} per accident searching the Internet with "r creating complex table". This package is not mentioned in the literature I have read so far, but it seems not only able to produce complex tables but is also used quite often. Maybe it is the HTML limitation which reduces it application for Online journals and probably require a separate step to produce {{< latex >}} tables ready for printing.

It is a pretty old package (version 1.0 appeared already in 2014) but still in development. The [last change](https://cran.r-project.org/web/packages/htmlTable/index.html) up today (2024-01-14) was 2023-10-29.
...
:::
::::{.my-note-container}
{**htmlTable**} seems an interesting package not only for creating complex tables but also for exporting them into Word. But this package is at the moment not on my task list.
::::
:::::
...
:::
::::{.my-note-container}
I have already some experiences with {**kableExtra**}. I used it quasi as an add-on to `knitr::kable()` during the first or second draft of my research report. I still have to see if other packages (like {**gt**}) not only has more features but are also easy to use.
::::
:::::
...
:::
::::{.my-note-container}
The package is completely new for me. I have to check it out to see how and when to use. But my priority now is {**gt**}.
::::
:::::
...
:::
::::{.my-note-container}
Learning {**xtable**} is not on my agenda: It is not clear for me if {**xtable**} is still in developments and if it is supporting newer approaches. Learning other table packages, especially {**gt**} together with {**gtsummary**} has higher priority for me.
::::
:::::
...
There are other packages as well. But the above seven/eight packages
are a first starting point for everybody to learn creating and displaying
sophisticated data tables in R. It is good to know about different packages but lastly I have to decide which one to learn and use in daily practice. At the moment my favorites are {**gt**} together with {**gtsummary**}.
...
As a consequence of my table packages review I experimented with the {**gtsummary**}. I wanted to create a table where I have information by the variables grass, age in four categories and gender with all their levels in absolute numbers and in percents.
...
chap01_gtsummary_basic <- chap01_nhanes_2013_clean |> 

    ## cut age into several defined age groups
    dplyr::mutate(age_cat = base::cut(age,
                   breaks = c(-Inf, 29, 39, 49, 59),
                   labels = c("18 - 29", "30 - 39", "40 - 49", "50 - 59"))) |> 
    dplyr::select(grass, age_cat, gender)
...
chap01_gtsummary_basic2 <- chap01_nhanes_2013_clean |> 
...
I am using both methods in following code chunk: I provided labelled metadata for the `grass` variable but used new variable names for `age_cat` and `gender`.
...
chap01_gtsummary_mod <-  chap01_gtsummary_basic |> 
...
chap01_gtsummary_cross1 <- chap01_gtsummary_basic |> 
    labelled::set_variable_labels(
            grass = "**Ever used grass or hash?**"
        ) |> 
    gtsummary::tbl_cross(
...
chap01_gtsummary_cross2 <- chap01_gtsummary_basic |> 
    labelled::set_variable_labels(
            grass = "**Ever used grass or hash?**"
        ) |> 
    gtsummary::tbl_cross(
...
gtsummary::tbl_stack(
    list(chap01_gtsummary_cross1,
         chap01_gtsummary_cross2)
    ) |> 
