...
2. **Check normality assumption**:
    - **Draw histograms of numeric variables**: To get a better idea I have these `r glossary("histogram", "histograms")` overlaid with the theoretical normal distributions and the `r glossary("density plots")` of the current data. The difference between these two curves gives a better impression if normality is met or not.
...
- The `r glossary("residuals")` are independent.
...
```{r}
#| label: diagnostics-standardize-residuals
#| results: hold
#| cache: true
...
Standardized `r glossary("residuals")` are `r glossary("z-score", "z-scores")` for the residual values. 
...
tbl_test <- distance_ssp_clean |> 
    dplyr::mutate(
        no_ins_log = car::logit(no_insurance),
        no_ins_arcsin = asin(sqrt(no_insurance / 100)),
        no_ins_sqrt = sqrt(no_insurance)
        )

shapiro.test(distance_ssp_clean$no_insurance)
shapiro.test(tbl_test$no_ins_log)
shapiro.test(tbl_test$no_ins_arcsin)
shapiro.test(tbl_test$no_ins_sqrt)

gg1 <- my_hist_dnorm(distance_ssp_clean, distance_ssp_clean$no_insurance)
gg2 <- my_hist_dnorm(tbl_test, tbl_test$no_ins_log)
gg3 <- my_hist_dnorm(tbl_test, tbl_test$no_ins_arcsin)
gg4 <- my_hist_dnorm(tbl_test, tbl_test$no_ins_sqrt)

patchwork:::"/.ggplot"(gg1, gg2)
patchwork:::"/.ggplot"(gg3, gg4)
...
{**broom**} converts untidy output into tidy tibbles. This is important whenever you want to combine results from multiple model (or tests). I have {**broom**} already used to compare results from the normality tests (`r glossary("Shapiro-Wilk")` and `r glossary("Anderson-Darling")`), and homogeneity tests (`r glossary("Levene")` and `r glossary("Fligner", "Fligner-Killeen")`. In the first case (normality) both tests result in `htest` objects. I could therefore bind as different rows in a new tibble dataframe (see: @tbl-chap09-eda-test-normality). In the second case (homogeneity) I got with `htest` and `anova` two different objects and had to work around the differences manually before I could bind them into the same tibble (See: @tbl-chap09-eda-test-homogeneity). 
