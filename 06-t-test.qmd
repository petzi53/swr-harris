...
We are following @prp-chap05-nhst from @sec-chap05-achievement5.
...
The formula is very similar as the `r glossary("Z-score")` statistic in @eq-z-score. The only difference is that in the above `r glossary("t-statistic")` the denominator is the `r glossary("standard error")` rather than the `r glossary("standard deviation")`.
