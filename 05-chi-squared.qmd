...
The probability that the differences between observed and expected values would result in a chi-squared of exactly 10 is pretty small. Looking into the data it would be around 2.8%. 

It might be more useful to know what the probability is of getting a chi-squared of 10 or higher. The probability of the chi-squared value being 10 or higher would be the area under the curve from 10 to the end of the distribution at the far right. 

While not as small as 2.8%, the probability of the chi-squared value being 10 or higher is still a relatively small number. The probability of the squared differences between observed and expected adding up to 10 or more is low when there is no relationship between the variables.

In this text case we would not have a statistically significant value, because the p-value is greater than 0.05. This can be seen clearly in another graph created with {**sjPlot**} (see @pak-sjPlot and the next tab).
...
chisq_ease_vote_stats <- stats::chisq.test(
    x = vote_clean$ease_vote,
    y = vote_clean$race
)

chisq_sjplot <- sjPlot::dist_chisq(
    chi2 = chisq_ease_vote_stats[["statistic"]], 
    deg.f = chisq_ease_vote_stats[["parameter"]])
...
- The limit $\chi^2$ where a statistically significant p-value < 0.05 would start is with `r chisq_sjplot[["plot_env"]][["cs"]]` much lower.
...
\end{align*}
$$ {#eq-adjusted-standardized-residuals}
...
:::::{.my-resource}
:::{.my-resource-header}
Function to get standardized residuals of chi-squared tests
:::
::::{.my-resource-container}
...
- `stats::chisq.test()$residuals`: (no data)
- `descr::CrossTable()`: It has arguments to show residuals, standardized residuals and adjusted standardized residuals  (500)
- `janitor::chisq.test(<tabyl>)$residuals` (3000)
- `questionr::chisq.residuals()` (300)
- `rstatix::pearson_residuals()`, `rstatix::std_residuals()` (7000)

There is also the possibility to use `graphics::mosaicplot()` with the option `shade = TRUE` to examine residuals for the source of differences (See [@greenwood2022]).
...
$$ {#eq-cramers-v-example-calculation}
...
::: {.callout-tip}
In this case, the effect size is between small and medium. There is a statistically significant relationship between opinions on voter registration and race-ethnicity, and the relationship is weak to moderate. This is consistent with the frequencies, which are different from expected, but not by an enormous amount in most of the groups.
:::
