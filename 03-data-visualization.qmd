...
waffle::waffle(parts = fired_2018,
               rows = 10,
               colors = grDevices::palette(my_colors)
)

ggplot2::ggplot(fired_2018,
    ggplot2::aes(fill = rounds_fired,
                 values = prop)) +
    waffle::geom_waffle() + 
...
nhanes_2012_clean |> 
    ...
    ## OPTION 1: Viridis colour scales from {viridisLite} using {ggplot2}
    ggplot2::scale_fill_viridis_d(
    ...
    # # OPTION 3: Viridis Color Scales for ggplot2 using {viridis}
    # viridis::scale_fill_viridis(
    #     begin = .25,
    #     end = .75,
    #     discrete = TRUE,
    ...
    # ## has no alpha, begin and end, 
    # ## needs base:rev() function for direction
    # ggplot2::scale_fill_manual(values =
...
::: my-r-code-container
```{r}
#| label: fig-mosaic-rounds-fired-sex-alpha
#| fig-cap: "Rounds fired by sex in the United States among 2011–2012 NHANES participants"
...
In this chapter I preferred "viridis::turbo"because it is the most colorful palette of {**viridis**}. But it turned out that the begin and end of the color scale are not distinguishable in black & white. I had therefore to use a different begin and/or end to prevent this similarity. 
::::
:::::
...
::: {#lst-chap03-practice-test}
```{r}
#| label: fig-color-replication
#| fig-cap: "Color replication of gun-use by sex"
#| fig-height: 3
#| results: hold

pal_data_1 <- list(names = c("Normal", "desaturated"),
...
pal_data_2 <- list(names = c("Normal", "desaturated"),
    color = list(scales::viridis_pal(
                                alpha = 1, 
                                begin = .15, 
                                 end = .35, 
...
list_plotter(pal_data_1$color, pal_data_1$names, 
     "Color used in my first try with gun-use by sex")
list_plotter(pal_data_2$color, pal_data_1$names, 
     "Changed colors to get better distinction with black & white printing")
```
Practice test: Replication of used color in @fig-mosaic-gun-use-sex
...
Read more details about {**viridis**} scales in the vignette [Introduction to the {**viridis**} color maps](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html). These scalles are not only colorful and pretty but also perceptually uniform and robust to color blindness (`r glossary("CVD")`). 
...
- If you do not use all colors you can exactly provide all parameters of your scale & color choice to `colorspace::desaturate()`. For instance: `colorspace::desaturate(viridis::turbo(5, 0.5, .25, .75, -1))` See for a practice test @lst-chap03-practice-test.
...
To visualize the effect of color blindness on our palettes we will turn to two packages. The `dichromat` package can simulate color blindness on individual colors or entire palettes.


:::::{.my-experiment}
:::{.my-experiment-header}
:::::: {#def-ID-text}
: Effects of color blindness on different palettes
::::::
:::
::::{.my-experiment-container}
...
```{r}
#| label: fig-cvd-bw-effects
#| fig-cap: "Color blindness and satured effects on different color palettes"
#| results: hold
...
pal_data <- list(names = c("Normal", "deuteranopia", "protanopia", "tritanopia"),
                 color = list(rainbow(256),
                              ...
                              colorspace::desaturate(rainbow(256))))

list_plotter(pal_data$color, pal_data$names, "The effect of color blindness on the rainbow palette")

pal_data <- list(names = c("Normal", "deuteranopia", "protanopia", "tritanopia", "saturated"),
                 color = list(viridis::magma(256),
                              dichromat::dichromat(viridis::magma(256), type = "deutan"),
                              dichromat::dichromat(viridis::magma(256), type = "protan"),
                              dichromat::dichromat(viridis::magma(256), type = "tritan"),
...
list_plotter(pal_data$color, pal_data$names, "Color blindness and satured effects on the viridis magma palette")
```
